...
DCA 复现代码与脚本。本仓库**不包含**完整 RL 训练框架、**不依赖也不 import verl/slime**，仅提供 DCA 优势计算。**main 分支**包含 DCA 核心与 **VERL / Slime 两种适配**：VERL 用户见 [docs/INTEGRATION_VERL.md](docs/INTEGRATION_VERL.md)，Slime 用户见 [docs/INTEGRATION_SLIME.md](docs/INTEGRATION_SLIME.md)。
...
**用 VERL 复现**：先 `python scripts/prepare_data.py --output_dir data/processed [--builtin_only]` 得到 `train.parquet`，再按 [docs/INTEGRATION_VERL.md](docs/INTEGRATION_VERL.md) patch VERL 后运行 `./scripts/run_verl_baselines.sh`。**Slime 用户**：按 [docs/INTEGRATION_SLIME.md](docs/INTEGRATION_SLIME.md) 在 [THUDM/slime](https://github.com/THUDM/slime) 中 patch 后运行 `./scripts/run_slime_baselines.sh`。已有结果时：`RESULTS_FILE=/path/to/results.jsonl ./scripts/run_full_pipeline.sh` 仅做评估。
...
├── dca/                 # 优势计算、指标、数据工具；verl_integration + slime_integration（main 两种适配）
├── scripts/
...
│   ├── run_slime_baselines.sh # Slime 三 baseline 对照
